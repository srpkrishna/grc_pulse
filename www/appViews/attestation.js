    ...
    var Attestation = React.createClass({displayName: "Attestation",
        ...
        render: function () {
            if (this.props.isMessage) {
                return (
                    React.createElement("div", {className: "messageDone", onClick: this._onMessageComplete}, getString("done"))
                );
            }
            else {
                var activeCss = (this.state.isActive ? "attestationBt attestationActive" : "attestationBt");
                return (
                    React.createElement("div", {className: "attestationContainer", id: "swip_to_attest", onClick: this._onClick}, 
                        React.createElement("div", {className: activeCss}, getString("swip_to_attest"))
                    )
                );
            }
        }
